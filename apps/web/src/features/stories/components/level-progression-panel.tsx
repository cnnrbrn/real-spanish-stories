import { Link } from '@tanstack/react-router'
import { STORY_LEVELS } from '@real-spanish-stories/shared'
import { Check } from 'lucide-react'
import { levelColorClasses, levelOutlineClasses } from '../utils/level-colors'
import { createStoryTitle } from '../utils/story'
import { getYouTubeThumbnail } from '../utils/video'
import type { StoryLevel, StoryResponse } from '@real-spanish-stories/shared'

interface LevelProgressionPanelProps {
  currentLevel: StoryLevel
  levels: StoryResponse[]
  altTitle: string
  /** 'compact' drops the thumbnails and story title for the in-page mobile card,
      where the <h1> directly above already names the story. */
  variant?: 'sidebar' | 'compact'
}

export function LevelProgressionPanel({ currentLevel, levels, altTitle, variant = 'sidebar' }: LevelProgressionPanelProps) {
  if (variant === 'compact') {
    return (
      <div>
        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3">Levels</p>
        <ul className="grid grid-cols-2 gap-2">
          {STORY_LEVELS.map(({ value, label }) => {
            const story = levels.find((s) => s.level === value)
            const isCurrent = value === currentLevel

            const content = (
              <div className={`flex h-full items-center justify-center gap-1.5 rounded-lg border-2 px-3 py-2.5 text-center text-sm leading-tight transition-colors ${isCurrent ? `${levelColorClasses[value]} border-transparent font-semibold` : `${levelOutlineClasses[value]} font-medium hover:bg-muted`} ${!story ? 'opacity-40' : ''}`}>
                {isCurrent && <Check className="w-4 h-4 shrink-0" />}
                {label}
              </div>
            )

            return (
              <li key={value}>
                {isCurrent || !story ? content : (
                  <Link to="/story/$slug" params={{ slug: story.slug }} className="block h-full">
                    {content}
                  </Link>
                )}
              </li>
            )
          })}
        </ul>
      </div>
    )
  }

  return (
    <div>
      <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-1">Levels</p>
      <h2 className="text-base font-semibold text-foreground mt-3 mb-3">{altTitle}</h2>
      <ul className="space-y-2">
        {STORY_LEVELS.map(({ value, label }) => {
          const story = levels.find((s) => s.level === value)
          const isCurrent = value === currentLevel
          const thumbnail = story?.videoLink ? getYouTubeThumbnail(story.videoLink, 'mqdefault') : null

          const content = (
            <div className={`flex items-center gap-3 rounded-lg p-1 -mx-1 transition-colors ${isCurrent ? 'bg-muted' : 'hover:bg-muted/60'} ${!story ? 'opacity-40' : ''}`}>
              <div className="w-20 aspect-video rounded overflow-hidden bg-muted shrink-0">
                {story && thumbnail && (
                  <img
                    src={thumbnail}
                    alt={createStoryTitle(story)}
                    loading="lazy"
                    className="w-full h-full object-cover"
                  />
                )}
              </div>
              <div className="min-w-0">
                <span className={`inline-block text-xs font-medium px-1.5 py-0.5 rounded mb-1 ${levelColorClasses[value]}`}>
                  {label}
                </span>
                <p className="text-xs text-foreground/80 truncate">
                  {story ? story.altTitle : 'Coming soon'}
                </p>
              </div>
            </div>
          )

          return (
            <li key={value}>
              {isCurrent || !story ? content : (
                <Link to="/story/$slug" params={{ slug: story.slug }}>
                  {content}
                </Link>
              )}
            </li>
          )
        })}
      </ul>
    </div>
  )
}
